...
export default function DeckSettingsV2() {
      ...
      <div className="min-h-screen bg-gradient-to-b from-zinc-950 via-zinc-900 to-zinc-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            ...
            className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white px-6 py-3 rounded-lg shadow-lg shadow-blue-500/20 transition-all"
          >
            Retour au dashboard
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-950 via-zinc-900 to-zinc-950 p-4 md:p-8">
      <div className="max-w-2xl mx-auto">
        {/* Header */}
        <div className="flex items-center gap-4 mb-6">
          <button
            onClick={() => router.push('/dashboard-entry')}
            className="text-zinc-400 hover:text-white transition-colors p-2 rounded-lg border border-zinc-700/50 hover:border-zinc-600/50 hover:bg-zinc-800/50"
          >
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Paramètres du deck</h1>
            <p className="text-zinc-400 text-sm mt-1">{deck?.name}</p>
          </div>
        </div>

        {/* Formulaire */}
        <div className="bg-zinc-900/80 backdrop-blur-sm rounded-xl p-6 border border-zinc-800/50">
          <div className="mb-6">
            ...
            <div className="space-y-3">
              <label
                className={`flex items-start gap-3 p-4 rounded-xl cursor-pointer border-2 transition-all duration-200 ${
                  learningMethod === 'IMMEDIATE'
                    ? 'bg-cyan-500/5 border-cyan-500/30 shadow-[0_0_15px_rgba(6,182,212,0.07)]'
          ...
          {error && (
            <div className="mb-6 p-4 bg-red-900/10 border border-red-700/30 rounded-xl">
          ...
          <div className="flex gap-3">
            <button
              onClick={handleSave}
              disabled={saving}
              className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-medium px-6 py-3 rounded-lg shadow-lg shadow-blue-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none"
            >
            ...
            <button
              onClick={() => router.push('/dashboard-entry')}
              disabled={saving}
              className="bg-zinc-800/50 border border-zinc-700/50 hover:bg-zinc-700/50 hover:border-zinc-600/50 text-zinc-300 font-medium px-6 py-3 rounded-lg transition-all disabled:opacity-50"
            >
              Annuler
            </button>
          </div>
