...
const BronzeMedalIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-amber-700">
    <path d="M7.21 15 2.66 7.14a2 2 0 0 1 .13-2.2L4.4 2.8A2 2 0 0 1 6 2h12a2 2 0 0 1 1.6.8l1.6 2.14a2 2 0 0 1 .14 2.2L16.79 15"></path>
    <path d="M11 12 5.12 2.2"></path>
    <path d="m13 12 5.88-9.8"></path>
    <path d="M8 7h8"></path>
...
interface FlashcardsLeaderboardEntry { rank: number; userId: string; displayName: string; reviewCount: number; }
interface MathLeaderboardEntry { rank: number; userId: string; displayName: string; bestScore: number; achievedAt: string; }
interface StreakLeaderboardEntry { rank: number; userId: string; displayName: string; currentStreak: number; maxStreak: number; }
interface FlashcardsLeaderboardData { leaderboard: FlashcardsLeaderboardEntry[]; currentUserId?: string; }
...
const LeaderboardCard: React.FC<LeaderboardCardProps> = ({ entry, dataType, isCurrentUser, streakMode }) => {
  ...
  const getRankBadge = (rank: number) => {
    if (rank === 1) return <span className="px-4 py-2 rounded-full bg-yellow-900/30 text-yellow-500 border border-yellow-700/50 font-semibold text-sm">Top 1</span>;
    if (rank === 2) return <span className="px-4 py-2 rounded-full bg-gray-900/30 text-gray-400 border border-gray-700/50 font-semibold text-sm">Top 2</span>;
    if (rank === 3) return <span className="px-4 py-2 rounded-full bg-orange-900/30 text-orange-500 border border-orange-700/50 font-semibold text-sm">Top 3</span>;
    return <span className="px-4 py-2 rounded-full bg-zinc-800/50 text-zinc-400 border border-zinc-700/50 font-semibold text-sm">#{rank}</span>;
  };
  ...
  return (
    <div className={`${cardBackground} border rounded-xl p-4 hover:bg-zinc-800/50 transition-colors flex items-center gap-4`}>
      <div className="flex-shrink-0">{getRankIcon(entry.rank)}</div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <h3 className="font-semibold text-lg text-white truncate">{entry.displayName}</h3>
        ...
        <div className="mb-6">
          <div className="bg-zinc-900/50 backdrop-blur-sm rounded-2xl p-1.5 border border-zinc-800 shadow-2xl">
            <div className="grid grid-cols-3 gap-1.5">
              <button onClick={() => setCategory('flashcards')} className={`px-6 py-4 rounded-xl font-semibold text-sm sm:text-base transition-all duration-200 ease-out ${category === 'flashcards' ? 'bg-gradient-to-r from-blue-600 to-blue-500 text-white shadow-lg shadow-blue-500/30 scale-[1.02]' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'}`}>
              ...
              <button onClick={() => setCategory('veryfastmath')} className={`px-6 py-4 rounded-xl font-semibold text-sm sm:text-base transition-all duration-200 ease-out ${category === 'veryfastmath' ? 'bg-gradient-to-r from-green-600 to-emerald-500 text-white shadow-lg shadow-green-500/30 scale-[1.02]' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'}`}>
              ...
              <button onClick={() => setCategory('streak')} className={`px-6 py-4 rounded-xl font-semibold text-sm sm:text-base transition-all duration-200 ease-out ${category === 'streak' ? 'bg-gradient-to-r from-orange-600 to-red-500 text-white shadow-lg shadow-orange-500/30 scale-[1.02]' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'}`}>
            ...
            <div className="bg-zinc-900/50 backdrop-blur-sm rounded-2xl p-1.5 border border-zinc-800">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                {(['ADDITION', 'SUBTRACTION', 'MULTIPLICATION', 'DIVISION'] as MathMode[]).map(mode => (
                  <button key={mode} onClick={() => setMathMode(mode)} className={`px-4 py-3 rounded-xl font-medium text-sm transition-all duration-200 ${mathMode === mode ? 'bg-gradient-to-r from-green-600 to-green-500 text-white shadow-md' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'}`}>
                    {getModeLabel(mode)}
                  </button>
                ))}
              </div>
          ...
          <div className="mb-6">
            <div className="bg-zinc-900/50 backdrop-blur-sm rounded-2xl p-1.5 border border-zinc-800">
              <div className="grid grid-cols-2 gap-1.5">
                <button onClick={() => setStreakMode('current')} className={`px-6 py-3 rounded-xl font-medium text-sm transition-all duration-200 ${streakMode === 'current' ? 'bg-gradient-to-r from-orange-600 to-orange-500 text-white shadow-md' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'}`}>🔥 Streak Actuel</button>
                <button onClick={() => setStreakMode('max')} className={`px-6 py-3 rounded-xl font-medium text-sm transition-all duration-200 ${streakMode === 'max' ? 'bg-gradient-to-r from-orange-600 to-orange-500 text-white shadow-md' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'}`}>🏆 Record Personnel</button>
              </div>
            </div>
          </div>
        )}

        {error && <div className="bg-red-900/50 border border-red-700 rounded-lg p-4 mb-8 text-red-200">{error}</div>}
