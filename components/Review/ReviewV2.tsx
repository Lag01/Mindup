...
export default function ReviewV2() {
  ...
  const handleRating = useCallback(async (rating: 'again' | 'hard' | 'good' | 'easy') => {
    ...
    try {
      ...
      if (learningMethod === 'ANKI') {
        ...
        // Retrait définitif de la carte : en mode ANKI/FSRS, une carte n'est montrée
        // qu'une seule fois par session. La note est enregistrée et FSRS replanifie la
        // carte (retour à la prochaine session / le lendemain). Aucune ré-insertion
        // intra-session : une session de N cartes reste une session de N cartes.
        const remainingQueue = cardQueue.slice(1);
        setCardQueue(remainingQueue);
        ...
        // POST en fire-and-forget : l'UI n'attend pas. En cas d'erreur réseau, la carte
        // (état serveur inchangé) sera reprise au prochain fetch, source de vérité.
        fetch('/api/review', {
          method: 'POST',
  ...
  // Pilote l'affichage du popup « terminé » (mode ANKI) : il s'affiche dès que la file
  // est vide. En l'absence de ré-insertion intra-session, une file vide signifie
  // définitivement la fin de session (aucune réponse de POST ne peut la repeupler).
  useEffect(() => {
  ...
  }, [cardQueue, loading, isStudyMode, learningMethod, deckId]);
